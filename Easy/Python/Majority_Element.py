class Solution:
    def majorityElement(self, nums: List[int]) -> int:
        max_count = 0 #Stores the highest reptition found from any one number.
...
#The other impotant thing is that after every single full loop, you have to check if the current count is greater than the highest recorded count. If so..
#..we replace the "max_count" variable with the current count, and we also record the value we were checking for during that loop as "value". In the end, we turn the "value" varaible.
...
#Step 2: First iterator is at 2 (index 1) and the second goes through 3,4,4,5,6. Count returned is 1, which isnt >1, so max_count is 1 and value is 1.
